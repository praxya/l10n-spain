...
class l10nEsAeatSii(models.Model):
    ...
    def action_active(self):
        if self.public_key:
            self.env.ref('l10n_es_aeat_sii.config_parameter_sii_crt').value = self.public_key
        if self.private_key:
            self.env.ref('l10n_es_aeat_sii.config_parameter_sii_key').value = self.private_key
